...
class NewtonPlot:
    ...
    def process_data(self, input_file):
        with open(input_file, mode='r') as file:
            ...
            line_skip_num = 6  # this is the number of the info block size. how many lines to skip for every read

            for i in range(0, len(lines), line_skip_num):
                fragments_line = lines[i]
                x_vel_line = lines[i + 1]
                y_vel_line = lines[i + 2]
                z_vel_line = lines[i + 3]
