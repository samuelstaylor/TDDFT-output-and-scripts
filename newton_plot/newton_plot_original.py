...
class NewtonPlot:
    ...
    def plot_3d_projections_1_limits(self, graph_name_tag="",
        ...
        # Scatter plot of the carbon velocities in 3D space
        ax1.scatter(H_X, H_Y, H_Z, c='r', marker='.', label='Hydrogen',
                    alpha=alpha)  # Plot with red dots, slightly transparent
        ax1.scatter(C_X, C_Y, C_Z, c='b', marker='.', label='Carbon',
                    alpha=alpha)  # Plot with blue dots, slightly transparent
        ax1.scatter(O_X, O_Y, O_Z, c='g', marker='.', label='Oxygen',
                    alpha=alpha)  # Plot with blue dots, slightly transparent
        ax1.scatter(N_X, N_Y, N_Z, c='y', marker='.', label='Nitrogen',
                    alpha=alpha)  # Plot with red dots, slightly transparent
        ...
        ax1.xaxis.set_minor_locator(
            MultipleLocator((abs(lim_3d_x_upper) + abs(lim_3d_x_lower)) / (2 * self.axis_subdivide)))
        ax1.yaxis.set_minor_locator(
            MultipleLocator((abs(lim_3d_y_upper) + abs(lim_3d_y_lower)) / (2 * self.axis_subdivide)))
        ax1.zaxis.set_minor_locator(
            MultipleLocator((abs(lim_3d_z_upper) + abs(lim_3d_z_lower)) / (2 * self.axis_subdivide)))
...
def main():
    ...
    if (data_mode.lower().startswith('q')):
        # QUANTUM INPUT FILE:
        input_file = 'newton_plot\\pyridazine\\moleculeFormations.csv'
