...
def process_fragments(filepath, line_skip_num=9):
    ...
    for i in range(0, len(lines), line_skip_num):
        ...
        speeds_line = lines[i + 7]  # speeds in A/fs
        ...
        speeds = [float(speed.strip()) for speed in speeds_line[1:] if speed.strip()]

        for fragment, density, speed in zip(fragments, densities, speeds):
            if fragment not in fragments_data:
                fragments_data[fragment] = []
                fragments_speed_data[fragment] = []
            fragments_data[fragment].append(density)
            fragments_speed_data[fragment].append(speed)

    print("Number of simulations:", (len(lines) / line_skip_num))
    return fragments_data, fragments_speed_data
...
def plot_charge_states(fragments_data_passed, fig_name="frag_charge_states.png"):
    ...
    for label, color in zip(labels, colors):
        if any(bar_data[label]):  # Check if any count is greater than zero
            bars = plt.bar(r, bar_data[label], color=color, edgecolor='white', width=bar_width, bottom=bottoms,
                           label=label)
    ...
    if legend_labels:  # Add legend only if there are labels to show
        plt.legend(title='Fragments Charge States', loc='upper right', title_fontsize='13', fontsize='11',
                   labels=legend_labels, labelcolor=legend_colors)
...
def plot_fragment_counts_and_averages_log(fragments_data, fig_name='frag_charge_averages.png', log_scale=True):
    ...
    bars_counts = plt.bar(sorted_fragments, sorted_counts, color='tab:blue', edgecolor='black', linewidth=.1,
                          label='Frequency')
    abs_averages = [abs(x) for x in sorted_averages]
    bars_averages = plt.bar(sorted_fragments, abs_averages, color='#b0dce4', edgecolor='black', linewidth=.1,
                            bottom=sorted_counts, label='Average Charge')
    ...
    for bar, count in zip(bars_counts, sorted_counts):
        ...
        plt.text(bar.get_x() + bar.get_width() / 2, height - height * 0.05, f'{count:.1f}', ha='center', va='top',
    ...
    for bar, average, count in zip(bars_averages, sorted_averages, sorted_counts):
        plt.text(bar.get_x() + bar.get_width() / 2, count + abs(average), f'{average:.1f}', ha='center', va='bottom',
                 fontweight='bold', color='black', fontsize=7)
...
def plot_fragment_counts_and_averages_two_ax(fragments_data, fig_name='frag_charge_averages.png',
                                             hydrogen_charge_scale_factor=10,
    ...
    ax1.set_ylabel('Hydrogen Frequency (%)', fontsize=16, fontweight='bold', color=hydrogen_freq_color,
    ...
    if hydrogen_index is not None:
        hydrogen_bars_count = ax1.bar(["H"], [hydrogen_count], color=hydrogen_freq_color, edgecolor='black',
                                      linewidth=.1, label='Hydrogen Frequency')
        if include_seperate_charge_leg_for_h:
            hydrogen_bars_average = ax1.bar(["H"], [abs(hydrogen_average_charge) * hydrogen_charge_scale_factor],
                                            color=hydrogen_charge_color, edgecolor='black', linewidth=.1,
                                            bottom=[hydrogen_count], label='Hydrogen Average Charge')
        else:
            hydrogen_bars_average = ax1.bar(["H"], [abs(hydrogen_average_charge) * hydrogen_charge_scale_factor],
                                            color=fragment_charge_color, edgecolor='black', linewidth=.1,
                                            bottom=[hydrogen_count])

        # Adding labels within the hydrogen bar for counts
        for bar in hydrogen_bars_count:
            height = bar.get_height()
            ax1.text(bar.get_x() + bar.get_width() / 2, height - height * 0.05, f'{hydrogen_count:.1f}', ha='center',
                     va='top', fontweight='bold', color='white', fontsize=inner_num_font_size,
                     fontname='Times New Roman')

        # Adding labels on top of the hydrogen bar for averages
        for bar in hydrogen_bars_average:
            ax1.text(bar.get_x() + bar.get_width() / 2,
                     hydrogen_count + (abs(hydrogen_average_charge) * hydrogen_charge_scale_factor),
                     f'{hydrogen_average_charge:.1f}', ha='center', va='bottom', fontweight='bold', color='black',
                     fontsize=inner_num_font_size, fontname='Times New Roman')

    # Create primary y-axis for other fragments
    ax2 = ax1.twinx()

    bars_counts = ax2.bar(sorted_fragments, sorted_counts, color=fragment_freq_color, edgecolor='black', linewidth=.1,
                          label='Fragment Frequency')
    abs_averages = [abs(x) for x in sorted_averages]
    bars_averages = ax2.bar(sorted_fragments, abs_averages, color=fragment_charge_color, edgecolor='black',
                            linewidth=.1, bottom=sorted_counts, label='Fragment Average Charge')

    ax2.set_ylabel('Frequency (%)', fontsize=16, fontweight='bold', color=fragment_freq_color,
                   fontname='Times New Roman')

    subscripted_fragments = [subscript_numbers(frag) for frag in sorted_fragments]
    # Set the tick positions and labels
    ax1.set_xticks(np.arange(len(subscripted_fragments)))  # Ensure the tick positions match the labels
    ax1.set_xticklabels(subscripted_fragments, rotation=45, ha="right", fontsize=14, fontweight='bold',
                        fontname='Times New Roman')

    ax1.tick_params(axis='y', which='both', labelsize=10, labelcolor=hydrogen_freq_color, direction='in', length=5,
                    width=1, colors='black', grid_color='gray', grid_alpha=0.5)
    ax2.tick_params(axis='y', which='both', labelsize=10, labelcolor=fragment_freq_color, direction='in', length=5,
                    width=1, colors='black', grid_color='gray', grid_alpha=0.5)
    ...
    for bar, count in zip(bars_counts, sorted_counts):
        height = bar.get_height()
        if height != 0:
            ax2.text(bar.get_x() + bar.get_width() / 2, height - height * 0.05, f'{count:.1f}', ha='center', va='top',
                     fontweight='bold', color='white', fontsize=inner_num_font_size, fontname='Times New Roman')

    # Adding labels on top of each bar for averages
    for bar, average, count in zip(bars_averages, sorted_averages, sorted_counts):
        height = bar.get_height()
        if height != 0:
            ax2.text(bar.get_x() + bar.get_width() / 2, count + abs(average), f'{average:.1f}', ha='center',
                     va='bottom', fontweight='bold', color='black', fontsize=inner_num_font_size,
                     fontname='Times New Roman')
...
def plot_hydrogen(fragments_data, fig_name='hydrogen_charge_distribution.png'):
    ...
    if hydrogen_key in fragments_data:
        ...
        plt.figure(figsize=(10, 6))
        counts, bins, patches = plt.hist(hydrogen_charges,
                                         bins=np.arange(min(hydrogen_charges), max(hydrogen_charges) + bin_size,
                                                        bin_size), edgecolor='black', color='tab:blue', density=True)
        ...
        for count, patch in zip(counts, patches):
            ...
            sum += count
            plt.text(patch.get_x() + patch.get_width() / 2, count, f'{count:.1f}', ha='center', va='bottom',
...
def plot_hydrogen_speed(fragments_speed_data, fig_name='hydrogen_speed_distribution.png'):
    hydrogen_key = "H"
    bin_size = 0.02

    plt.rcParams['font.family'] = 'Times New Roman'

    if hydrogen_key in fragments_speed_data:
        hydrogen_speeds = fragments_speed_data[hydrogen_key]

        # Create the histogram
        plt.figure(figsize=(10, 6))
        counts, bins, patches = plt.hist(hydrogen_speeds,
                                         bins=np.arange(min(hydrogen_speeds), max(hydrogen_speeds) + bin_size,
                                                        bin_size), edgecolor='black', color='tab:red', density=True)

        # Convert counts to percentage
        counts = counts * bin_size * 100

        sum_freq = 0
        # Plot the histogram with the speeds
        for count, patch in zip(counts, patches):
            patch.set_height(count)
            sum_freq += count
            plt.text(patch.get_x() + patch.get_width() / 2, count, f'{count:.1f}', ha='center', va='bottom',
        ...
        print("Sum of speeds for hydrogen histogram:", sum_freq)

        # Set x-ticks and labels for each bar
        bin_labels = [f'{bins[i]:.2f}' for i in range(len(bins))]
        plt.xticks(bins, bin_labels, rotation=45, fontsize=14)  # Increased font size
        plt.tick_params(axis='y', direction='in', labelsize=14)  # Increased font size

        plt.xlabel('Speed (Å/fs)', fontsize=18, fontweight='bold')
        plt.ylabel('Hydrogen Frequency (%)', fontsize=18, fontweight='bold')
        #plt.title('Distribution of Hydrogen Speeds', fontsize=17, fontweight='bold')
        plt.ylim(0, max(counts) * 1.1)
        ...
        print("No hydrogen speed data available.")


def plot_hydrogen_ke(fragments_speed_data, fig_name='hydrogen_ke_distribution.png'):
    hydrogen_key = "H"
    bin_size = 0.5
    ...
    if hydrogen_key in fragments_speed_data:
        ...
        # Create the histogram
        plt.figure(figsize=(10, 6))
        counts, bins, patches = plt.hist(hydrogen_ke,
                                         bins=np.arange(min(hydrogen_ke), max(hydrogen_ke) + bin_size,
                                                        bin_size), edgecolor='black', color='tab:red', density=True)
        ...
        print("Sum of ke for hydrogen histogram:", sum_freq)
        ...
        plt.xlabel('Kinetic Energy (eV)', fontsize=18, fontweight='bold')
        plt.ylabel('Hydrogen Frequency (%)', fontsize=18, fontweight='bold')
        #plt.title('Distribution of Kinetic Energy of Hydrogen', fontsize=17, fontweight='bold')
        plt.ylim(0, max(counts) * 1.1)
        ...
        print("No hydrogen ke data available.")
...
def main():
    ...
    input_file_path = r'C:\Users\Hirom\OneDrive\Vanderbilt\Kalman research\TDDFT-output-and-scripts\histograms_stats_molecule_formation\cyclobutane_7.50\moleculeFormations.csv'
    ...
    plot_fragment_counts_and_averages_two_ax(fragments_data, fig_name=os.path.join(output_file_directory,
                                                                                   "frag_charge_averages_two_axes.png"),
    ...
    plot_hydrogen_speed(fragments_speed_data, fig_name=os.path.join(output_file_directory, 'hydrogen_speed_distribution.png'))
    plot_hydrogen_ke(fragments_speed_data, fig_name=os.path.join(output_file_directory, 'hydrogen_ke_distribution.png'))
